...
*   **Batch-First 模式**：自 1.1.0 版本起，全面转向批量处理 (`process_batch`)，极大提升了 API 调用效率。
*   **上下文感知**：内置进度汇报、Token 审计与动态取消信号，完美契合 LLM 调用场景。

### ⚡ 极致性能优化 (Performance First)
*   **高效并行调度**：支持全局并行调度模型，彻底压榨 CPU 与 I/O 潜力。
...
*   🚀 **高效并行调度**：自动适配 I/O 密集（LLM 调用）与 CPU 密集（文本清洗）任务。
...
*   **LLM 语料清洗**：利用并行架构与流式传输，快速处理海量文本数据。
