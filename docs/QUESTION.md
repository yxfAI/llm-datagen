...
*   **现象**：流式恢复时，下游节点进度显示 `10/10` 且不再更新，实际上游还有新数据。
...
### Q19: 如何保证算子的多态性与调用健壮性？
*   **痛点**：自 1.1.0 版本起，框架全面转向 **Batch-First** 模式。
*   **解决方案**：
    1.  **统一批量容器**：引入 `OperatorNode` 和 `ParallelOperatorNode`。它们不再区分单条/批量算子，而是强制要求算子实现 `process_batch`。
    2.  **基类规范**：提供 `BaseOperator` 基类，强制开发者实现 `process_batch` 以获得最佳性能。
*   **价值**：消除了多层嵌套逻辑，提升了框架的整体执行效率。
...
### Q8: 为什么我使用了持久化协议却无法恢复进度？
*   **核心原则**：恢复能力是由物理存储协议（如 `jsonl://`）和 `pipeline_id` 共同决定的，无需显式开关。
*   **排查步骤**：
    1.  **检查 Pipeline ID**：恢复运行必须传入与崩溃前完全一致的 `pipeline_id`。
    2.  **检查存储协议**：只有 `jsonl://` 和 `csv://` 等持久化协议支持恢复，`memory://` 是纯内存的，进程结束即销毁。
    3.  **检查 .done 文件**：如果输出目录已经存在 `.done` 文件，说明该节点已被标记为完成。若需重跑，请删除该文件或更换 `pipeline_id`。
...
| **算子契约** | **Batch-First** (批量优先) | 产生性能瓶颈或多层包装损耗 |